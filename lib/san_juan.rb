    ...
    namespace :god do
      ...
      namespace role do
        ...
        task :start, :roles => role do
          try_sudo "cd #{current_path}; #{fetch(:god, "god")} -c #{san_juan.configuration_path(current_path, role)}"
        end

        desc "Start god interactively"
        task :start_interactive, :roles => role do
          try_sudo "cd #{current_path}; #{fetch(:god, "god")} -c #{san_juan.configuration_path(current_path, role)} -D"
        end

        desc "Reload the god config file"
        task :reload, :roles => role do
          try_sudo "cd #{current_path}; #{fetch(:god, "god")} load #{san_juan.configuration_path(current_path, role)}"
        end

        desc "Quit god, but not the processes it's monitoring"
        task :quit, :roles => role do
          try_sudo "cd #{current_path}; #{fetch(:god, "god")} quit"
        end

        desc "Terminate god and all monitored processes"
        task :terminate, :roles => role do
          try_sudo "cd #{current_path}; #{fetch(:god, "god")} terminate"
        end

        desc "Describe the status of the running tasks"
        task :status, :roles => role do
          try_sudo "cd #{current_path}; #{fetch(:god, "god")} status"
        end
          ...
          namespace watch do
            %w(start restart stop unmonitor remove log).each do |command|
              ...
              task command, :roles => role do
                try_sudo "cd #{current_path}; #{fetch(:god, "god")} #{command} #{watch}"
              end
            end
          end
        end
